...
const j = { Normal: 1, Surname: 10, Custom: 100 };
function d(n) {
  var i;
  return (
    n.length -
    (((i = n.match(X)) === null || i === void 0 ? void 0 : i.length) || 0)
  );
}
function x(n) {
  const i = [];
...
const Z = gn(),
  ...
  on = Object.keys(F);
function zn(n, i, h) {
  if (on.indexOf(n) === -1) return w(n);
  if (i === h && i && w(i) !== i) return q(w(n));
  if (h && !sn[n].includes(h)) {
    const u = w(h);
    if (u !== h) {
      const g = p(u),
        s = F[n];
      for (let o in s) if (s[o].indexOf(Number(g)) !== -1) return o;
    }
  }
}
function ln(n, i) {
  if (n === "了" && (!i || !b.get(i))) return "liǎo";
}
function yn(n, i) {
  if (n === "々") return !i || !b.get(i) ? "tóng" : b.get(i).split(" ")[0];
}
function M(n, i, h) {
  return yn(n, i) || ln(n, i) || zn(n, i, h) || w(n);
}
const k = {
    南宫: "nán gōng",
    第五: "dì wǔ",
...
function bn(n) {
  return n.priority === j.Custom
    ? -(n.length * n.length * 100)
    : n.priority === j.Surname
...
function xn(n, i) {
  ...
  for (let s = i - 1; s >= 0; s--) {
    ...
    for (; g && g.index + g.length - 1 === s; ) {
      const y = g.index,
        c = {
          probability: g.probability * o.probability,
          decimal: o.decimal + bn(g),
          patterns: o.patterns,
          concatPattern: g
        };
      E(c), (h[y] = A(h[y], c)), (g = n[--u]);
    }
    const z = {
...
function wn(n, i) {
  ...
  let u = n.length - 1,
    g = n[u];
  for (let s = i - 1; s >= 0; s--) {
    const o = s + 1 >= i ? { count: 0, patterns: [] } : h[s + 1];
    for (; g && g.index + g.length - 1 === s; ) {
      const y = g.index,
        c = { count: rn(g) + o.count, patterns: o.patterns, concatPattern: g };
      (h[y] = O(h[y], c)), (g = n[--u]);
    }
...
function mn(n) {
  ...
  for (let h = n.length - 1; h >= 0; ) {
    const { index: u } = n[h];
    let g = h - 1;
    for (; g >= 0 && fn(n[h], n[g]); ) g--;
    (g < 0 || n[g].index + n[g].length <= u) && i.push(n[h]), (h = g);
  }
  return i.reverse();
}
var L;
...
class qn {
  ...
  buildTrie(i) {
    for (let h of i) {
      ...
      for (let s = 0; s < u.length; s++) {
        let o = u[s];
        if (!g.children.has(o)) {
          const z = new R(g, u.slice(0, s).join(""), o);
          g.children.set(o, z), this.addNodeToQueues(z);
        }
        g = g.children.get(o);
      }
      this.insertPattern(g.patterns, h),
        (h.node = g),
  ...
  buildFailPointer() {
    ...
    for (
      ...
      let u = i[h++],
        g = u.parent && u.parent.fail,
        s = u.key;
      for (; g && !g.children.has(s); ) g = g.fail;
      g ? (u.fail = g.children.get(s)) : (u.fail = this.root);
    }
  }
  addPatternToDictMap(i) {
    this.dictMap.has(i.dict) || this.dictMap.set(i.dict, new Set()),
      this.dictMap.get(i.dict).add(i);
  }
  addNodeToQueues(i) {
    this.queues[d(i.prefix)] || (this.queues[d(i.prefix)] = []),
      this.queues[d(i.prefix)].push(i);
  }
  insertPattern(i, h) {
    for (let u = i.length - 1; u >= 0; u--) {
      const g = i[u];
      if (h.priority === g.priority && h.probability >= g.probability)
        i[u + 1] = g;
      else if (h.priority > g.priority) i[u + 1] = g;
      else {
        i[u + 1] = h;
        return;
      }
  ...
  removeDict(i) {
    this.dictMap.has(i) &&
      (this.dictMap.get(i).forEach(u => {
        u.node.patterns = u.node.patterns.filter(g => g !== u);
      }),
  ...
  match(i, h) {
    ...
    for (let o = 0; o < s.length; o++) {
      let z = s[o];
      for (; u !== null && !u.children.has(z); ) u = u.fail;
      if (u === null) u = this.root;
      else {
        u = u.children.get(z);
        const y = u.patterns.find(l =>
          h === "off"
            ? l.priority !== j.Surname
            : h === "head"
              ? l.length - 1 - o === 0
              : !0
        );
        y &&
          g.push(
            Object.assign(Object.assign({}, y), { index: o - y.length + 1 })
          );
        let c = u.fail;
        for (; c !== null; ) {
          const l = c.patterns.find(t =>
            h === "off"
              ? t.priority !== j.Surname
              : h === "head"
                ? t.length - 1 - o === 0
                : !0
          );
          l &&
            g.push(
              Object.assign(Object.assign({}, l), { index: o - l.length + 1 })
            ),
            (c = c.fail);
        }
      }
  ...
  search(i, h, u = 2) {
    const g = this.match(i, h);
    return u === 1 ? mn(g) : u === 3 ? wn(g, d(i)) : xn(g, d(i));
  }
}
const pn = [...en, ...dn, ...jn, ...tn, ...un, ...cn],
  $ = new qn();
$.build(pn);
const an = new W(),
  kn = () => an,
  ...
  Cn = (n, i, h, u) => {
    const g = $.search(n, h, u);
    let s = 0;
    const o = x(n);
    for (let z = 0; z < o.length; ) {
      const y = g[s];
      if (y && z === y.index) {
        if (y.length === 1 && y.priority <= j.Normal) {
          const t = o[z];
          let e = "";
          (e = M(t, o[z - 1], o[z + 1])),
            (i[z] = { origin: t, result: e, isZh: e !== t, originPinyin: e }),
            z++,
            s++;
          continue;
        }
        const c = y.pinyin.split(" ");
        let l = 0;
        ...
        let l = "";
        (l = M(c, o[z - 1], o[z + 1])),
          (i[z] = { origin: c, result: l, isZh: l !== c, originPinyin: l }),
          z++;
      }
    }
    return { list: i, matches: g };
  },
  q = n =>
    n
      .replace(/(ā|á|ǎ|à)/g, "a")
  ...
  B = (n, i = "off") => {
    const h = kn();
    let u = b.get(n) ? b.get(n).split(" ") : [];
    if (h.get(n)) u = h.get(n).split(" ");
    else if (i !== "off") {
      const g = k[n];
      g && (u = [g].concat(u.filter(s => s !== g)));
    }
    return u;
  },
  Pn = (n, i = "off") => {
    let h = B(n, i);
    return h.length > 0
      ? h.map(u => ({ origin: n, result: u, isZh: !0, originPinyin: u }))
  ...
  m = n => {
    ...
    for (let g of i)
      for (let s of Y)
        if (g.startsWith(s)) {
          let o = g.slice(s.length);
          J.indexOf(s) !== -1 && K.indexOf(o) !== -1 && (o = nn[o]),
            h.push(s),
            u.push(o);
          break;
        }
    return { final: u.join(" "), initial: h.join(" ") };
  },
  f = n => {
    const { final: i } = m(n);
    let h = "",
    ...
    return (
      hn.indexOf(q(i)) !== -1
        ? ((h = i[0]), (u = i[1]), (g = i.slice(2)))
        : ((u = i[0] || ""), (g = i.slice(1) || "")),
      { head: h, body: u, tail: g }
    );
  },
  p = n => {
    const i = /(ā|ō|ē|ī|ū|ǖ|n̄|m̄|ê̄)/,
      h = /(á|ó|é|í|ú|ǘ|ń|ḿ|ế)/,
      u = /(ǎ|ǒ|ě|ǐ|ǔ|ǚ|ň|m̌|ê̌)/,
      g = /(à|ò|è|ì|ù|ǜ|ǹ|m̀|ề)/,
      s = /(a|o|e|i|u|ü|ê)/,
      o = /(n|m)$/,
      z = [];
    return (
...
const Tn = (n, i) => {
    let h = i.nonZh;
    if (h === "removed")
      return n.filter(u => u.isZh || !a(u.origin, i.nonZhScope));
    if (h === "consecutive") {
      for (let u = n.length - 2; u >= 0; u--) {
        ...
        !g.isZh &&
          !s.isZh &&
          a(g.origin, i.nonZhScope) &&
          a(s.origin, i.nonZhScope) &&
          ((g.origin += s.origin), (g.result += s.result), (s.delete = !0));
      }
      return n.filter(u => !u.delete);
    } else return n;
  },
  U = (n, i) => (d(n) === 1 && i.multiple ? Pn(n, i.surname) : !1),
  Zn = (n, i) => {
    switch (i.pattern) {
      case "pinyin":
        break;
      case "num":
        n.forEach(h => {
          h.result = h.isZh ? p(h.result) : "";
        });
        break;
      case "initial":
        n.forEach(h => {
          h.result = h.isZh ? m(h.result).initial : "";
        });
        break;
      case "final":
        n.forEach(h => {
          h.result = h.isZh ? m(h.result).final : "";
        });
        break;
      case "first":
        n.forEach(h => {
          h.result = H(h.result, h.isZh);
        });
        break;
      case "finalHead":
        n.forEach(h => {
          h.result = h.isZh ? f(h.result).head : "";
        });
        break;
      case "finalBody":
        n.forEach(h => {
          h.result = h.isZh ? f(h.result).body : "";
        });
        break;
      case "finalTail":
        n.forEach(h => {
          h.result = h.isZh ? f(h.result).tail : "";
        });
        break;
    }
  },
  Mn = (n, i) => {
    switch (i.toneType) {
      ...
      case "none":
        n.forEach(h => {
          h.isZh && (h.result = q(h.result));
        });
        break;
      case "num": {
        n.forEach(h => {
          h.isZh && (h.result = Sn(h.result, h.originPinyin));
        });
        break;
      }
    }
  ...
  In = (n, i) => {
    i.v &&
      n.forEach(h => {
        h.isZh && (h.result = h.result.replace(/ü/g, "v"));
      });
  },
  vn = (n, i, h) => {
    if (i.multiple && d(h) === 1) {
    ...
    return i.type === "array"
      ...
      : i.type === "all"
        ? n.map(u => {
            const g = u.isZh ? u.result : "",
              { initial: s, final: o } = m(g),
              { head: z, body: y, tail: c } = f(g);
            let l = [];
            return (
              g !== "" &&
                (l = [g].concat(B(u.origin, i.surname).filter(t => t !== g))),
              {
                origin: u.origin,
                pinyin: g,
                initial: s,
                final: o,
                first: H(u.result, u.isZh),
                finalHead: z,
                finalBody: y,
                finalTail: c,
                num: Number(p(u.originPinyin)),
                isZh: u.isZh,
                polyphonic: l,
                inZhRange: !!b.get(u.origin),
                result: u.result
              }
            );
          })
...
function Q(n, i) {
  if (((i = Object.assign(Object.assign({}, _n), i || {})), !Dn(n))) return n;
  ...
  let u = Array(d(n)),
    { list: g } = Cn(n, u, i.surname, i.segmentit);
  return (
    (g = Nn(g, i.toneSandhi)),
    (g = Tn(g, i)),
    U(n, i) && (g = U(n, i)),
...
const An = {
  ...
  Rn = (n, i, h) => {
    ...
    u.insensitive !== !1 && ((n = n.toLowerCase()), (i = i.toLowerCase())),
      u.space === "ignore" && (i = i.replace(/\s/g, ""));
    const g = On(n, i, u);
    return Ln(n, g);
  },
  On = (n, i, h) => {
    ...
    for (let s = 0; s < g.length; s++)
      (g[s] = Array(i.length + 1)), (g[s][0] = []);
    for (let s = 0; s < g[0].length; s++) g[0][s] = [];
    for (let s = 1; s < g.length; s++) {
      if (!h.continuous || (h.space == "ignore" && u[s - 1] === " "))
        for (let o = 1; o <= i.length; o++) g[s][o - 1] = g[s - 1][o - 1];
      for (let o = 1; o <= i.length; o++)
        if (g[s - 1][o - 1]) {
          if (o !== 1 && !g[s - 1][o - 1].length) continue;
          {
            const z = Q(u[s - 1], {
              type: "array",
              toneType: "none",
            ...
            if (u[s - 1] === i[o - 1]) {
              const l = [...g[s - 1][o - 1], s - 1];
              if (
                ((!g[s][o] || l.length > g[s][o].length) && (g[s][o] = l),
                o === i.length)
              )
                return g[s][o];
            }
            if (
              ...
              z.some(t =>
                h.lastPrecision === "any"
                  ? t.includes(i.slice(o - 1, i.length))
                  : h.lastPrecision === "start"
                    ? t.startsWith(i.slice(o - 1, i.length))
                    : h.lastPrecision === "first"
                      ? t[0] === i.slice(o - 1, i.length)
                      : h.lastPrecision === "every"
                        ? t === i.slice(o - 1, i.length)
                        : !1
              )
            )
              return [...g[s - 1][o - 1], s - 1];
            const y = h.precision;
            if (
              (y === "start" &&
                z.forEach(l => {
                  let t = o;
                  const e = [...g[s - 1][o - 1], s - 1];
                  for (; t <= i.length && l.startsWith(i.slice(o - 1, t)); )
                    (!g[s][t] || e.length > g[s][t].length) && (g[s][t] = e),
                      t++;
                }),
              y === "first" && z.some(l => l[0] === i[o - 1]))
            ) {
              const l = [...g[s - 1][o - 1], s - 1];
              (!g[s][o] || l.length > g[s][o].length) && (g[s][o] = l);
            }
            const c = z.find(l => l === i.slice(o - 1, o - 1 + l.length));
            if (c) {
              const l = [...g[s - 1][o - 1], s - 1],
                t = o - 1 + c.length;
              (!g[s][t] || l.length > g[s][t].length) && (g[s][t] = l);
            }
          }
        } else continue;
    }
...
function Ln(n, i) {
  ...
  for (let o = 0; o < i.length; o++) {
    const z = i[o];
    for (; s <= z; ) g[s].length === 2 && u++, s++;
    const y = z + u;
    g[z].length === 2 ? h.push(y - 1, y) : h.push(y);
  }
  return h;
}
const r = {
  ...
  Un = (n, i) => {
    const h = Object.assign(Object.assign({}, r), i || {});
    return Q(n, {
      type: "all",
      toneType: h.toneType,
      toneSandhi: i == null ? void 0 : i.toneSandhi
    })
      .map(s => {
        let o = "";
        for (const z in h.customClassMap)
          h.customClassMap[z].includes(s.origin) && (o += ` ${z}`);
        if (s.isZh) {
          const z = h.resultClass || r.resultClass,
            y = h.chineseClass || r.chineseClass,
            c = h.pinyinClass || r.pinyinClass;
          return `<span class="${z}${o}"><ruby><span class="${y}">${s.origin}</span>${h.rp ? "<rp>(</rp>" : ""}<rt class="${c}">${s.pinyin}</rt>${h.rp ? "<rp>)</rp>" : ""}</ruby></span>`;
        } else
          return h.wrapNonChinese
            ? `<span class="${h.nonChineseClass || r.nonChineseClass}${o}">${s.origin}</span>`
            : s.origin;
      })
      .join("");
  };
