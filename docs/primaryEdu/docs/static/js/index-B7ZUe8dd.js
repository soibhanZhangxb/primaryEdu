...
var Pt = (r, o, i) =>
    o in r
      ? de(r, o, { enumerable: !0, configurable: !0, writable: !0, value: i })
      : (r[o] = i),
  Mt = (r, o) => {
    for (var i in o || (o = {})) ge.call(o, i) && Pt(r, i, o[i]);
    if (It) for (var i of It(o)) he.call(o, i) && Pt(r, i, o[i]);
    return r;
  };
var tt = (r, o, i) =>
  new Promise((n, t) => {
    var e = u => {
      ...
      a = u => (u.done ? n(u.value) : Promise.resolve(u.value).then(e, s));
    a((i = i.apply(r, o)).next());
  });
import { p as W } from "./propTypes-B9jTUFW5.js";
import {
  R as me,
  d as we,
  a as Nt,
...
import { R as be } from "./refresh-right-BB4FT_tY.js";
var j = {},
  et,
  Dt;
function Se() {
  ...
  return (
    Gt ||
      ...
      (function (r) {
        ...
        function u(E, g, P) {
          for (let f = 1; f <= 40; f++)
            if (g <= r.getCapacity(f, P, E)) return f;
        }
        function c(E, g) {
        ...
        (r.from = function (g, P) {
          ...
          (r.getCapacity = function (g, P, f) {
            if (!e.isValid(g)) throw new Error("Invalid QR Code version");
            typeof f == "undefined" && (f = t.BYTE);
            const M = o.getSymbolTotalCodewords(g),
              h = i.getTotalCodewordsCount(g, P),
              b = (M - h) * 8;
            if (f === t.MIXED) return b;
            const R = b - c(f, g);
            switch (f) {
              case t.NUMERIC:
          ...
          (r.getBestVersionForData = function (g, P) {
            ...
            if (Array.isArray(g)) {
              if (g.length > 1) return S(g, M);
              if (g.length === 0) return 1;
              f = g[0];
            } else f = g;
            return u(f.mode, f.getLength(), M);
          }),
      ...
      (function (r) {
        var o = {
          single_source_shortest_paths: function (i, n, t) {
            ...
            a.push(n, 0);
            for (var u, c, m, S, E, g, P, f, M; !a.empty(); ) {
              (u = a.pop()), (c = u.value), (S = u.cost), (E = i[c] || {});
              for (m in E)
                E.hasOwnProperty(m) &&
                  ((g = E[m]),
                  (P = S + g),
                  (f = s[m]),
                  (M = typeof s[m] == "undefined"),
                  (M || f > P) && ((s[m] = P), a.push(m, P), (e[m] = c)));
            }
...
function Ke() {
  return (
    ee ||
      ...
      (function (r) {
        ...
        function S(h) {
          const b = m(s.NUMERIC, o.NUMERIC, h),
            R = m(s.ALPHANUMERIC, o.ALPHANUMERIC, h);
          let C, N;
          return (
            a.isKanjiModeEnabled()
              ? ((C = m(s.BYTE, o.BYTE, h)), (N = m(s.KANJI, o.KANJI, h)))
              : ((C = m(s.BYTE_KANJI, o.BYTE, h)), (N = [])),
            b
              .concat(R, C, N)
              .sort(function (p, A) {
          ...
          for (let R = 0; R < h.length; R++) {
            ...
            switch (C.mode) {
              case o.NUMERIC:
                ...
                break;
              case o.ALPHANUMERIC:
                b.push([C, { data: C.data, mode: o.BYTE, length: C.length }]);
                break;
              case o.KANJI:
                b.push([C, { data: C.data, mode: o.BYTE, length: c(C.data) }]);
                break;
              case o.BYTE:
                b.push([{ data: C.data, mode: o.BYTE, length: c(C.data) }]);
            }
          }
          return b;
        }
        function f(h, b) {
          ...
          for (let w = 0; w < h.length; w++) {
            ...
            for (let l = 0; l < p.length; l++) {
              const y = p[l],
                B = "" + w + l;
              A.push(B), (R[B] = { node: y, lastCount: 0 }), (C[B] = {});
                ...
                R[d] && R[d].node.mode === y.mode
                  ...
                  : (R[d] && (R[d].lastCount = y.length),
                    (C[d][B] =
                      E(y.length, y.mode) +
                      4 +
                      o.getCharCountIndicator(y.mode, b)));
              }
            }
            N = A;
          }
          for (let w = 0; w < N.length; w++) C[N[w]].end = 0;
          return { map: C, table: R };
        }
        function M(h, b) {
          let R;
          const C = o.getBestModeForData(h);
          if (((R = o.from(b, C)), R !== o.BYTE && R.bit < C.bit))
            throw new Error(
              '"' +
                h +
 ...
 Suggested mode is: ` +
        ...
        (r.fromArray = function (b) {
          return b.reduce(function (R, C) {
            return (
              typeof C == "string"
                ? R.push(M(C, null))
                : C.data && R.push(M(C.data, C.mode)),
              R
            );
          }, []);
        }),
          (r.fromString = function (b, R) {
            const C = S(b, a.isKanjiModeEnabled()),
              N = P(C),
              w = f(N, R),
              p = u.find_path(w.map, "start", "end"),
              A = [];
            for (let l = 1; l < p.length - 1; l++) A.push(w.table[p[l]].node);
            return r.fromArray(g(A));
          }),
          (r.rawSplit = function (b) {
            return r.fromArray(S(b, a.isKanjiModeEnabled()));
          });
      })(mt)),
    mt
  );
...
function Oe() {
  ...
  function P(w) {
    ...
    for (let A = 8; A < p - 8; A++) {
      const l = A % 2 === 0;
      w.set(A, 6, l, !0), w.set(6, A, l, !0);
    }
  }
  function f(w, p) {
    const A = t.getPositions(p);
    for (let l = 0; l < A.length; l++) {
      const y = A[l][0],
        B = A[l][1];
      for (let T = -2; T <= 2; T++)
        for (let d = -2; d <= 2; d++)
          T === -2 || T === 2 || d === -2 || d === 2 || (T === 0 && d === 0)
            ? w.set(y + T, B + d, !0, !0)
            : w.set(y + T, B + d, !1, !0);
    }
  }
  ...
  function R(w, p, A) {
    ...
    A.forEach(function (I) {
      l.put(I.mode.bit, 4),
        l.put(I.getLength(), S.getCharCountIndicator(I.mode, w)),
        I.write(l);
    });
    const y = r.getSymbolTotalCodewords(w),
      ...
      T = (y - B) * 8;
    for (
      l.getLengthInBits() + 4 <= T && l.put(0, 4);
      l.getLengthInBits() % 8 !== 0;

    )
      l.putBit(0);
    const d = (T - l.getLengthInBits()) / 8;
    for (let I = 0; I < d; I++) l.put(I % 2 ? 17 : 236, 8);
    return C(l, w, p);
  }
  function C(w, p, A) {
    const l = r.getSymbolTotalCodewords(p),
      y = a.getTotalCodewordsCount(p, A),
      B = l - y,
      T = a.getBlocksCount(p, A),
      d = l % T,
      I = T - d,
      v = Math.floor(l / T),
      k = Math.floor(B / T),
    ...
    const G = new Array(T),
      bt = new Array(T);
    let X = 0;
    const fe = new Uint8Array(w.buffer);
    for (let J = 0; J < T; J++) {
      const x = J < I ? k : F;
      (G[J] = fe.slice(O, O + x)),
        (bt[J] = Q.encode(G[J])),
    ...
    let St = 0,
      ...
      q;
    for (D = 0; D < X; D++)
      for (q = 0; q < T; q++) D < G[q].length && (Z[St++] = G[q][D]);
    for (D = 0; D < z; D++) for (q = 0; q < T; q++) Z[St++] = bt[q][D];
    return Z;
  }
  function N(w, p, A, l) {
    let y;
    if (Array.isArray(w)) y = E.fromArray(w);
      ...
      if (!v) {
        ...
        v = c.getBestVersionForData(k, A);
      }
      y = E.fromString(w, v || 40);
    } else throw new Error("Invalid data");
    const B = c.getBestVersionForData(y, A);
    if (!B)
      throw new Error(
...
`
    ...
    return (
      ...
      p >= 7 && M(I, p),
      b(I, T),
      isNaN(l) && (l = s.getBestMask(I, h.bind(null, I, A))),
      s.applyMask(l, I),
      h(I, A, l),
      {
        modules: I,
  ...
  return (
    (nt.create = function (p, A) {
      if (typeof p == "undefined" || p === "") throw new Error("No input text");
      let l = o.M,
        y,
        B;
      return (
        typeof A != "undefined" &&
          ((l = o.from(A.errorCorrectionLevel, o.M)),
          (y = c.from(A.version)),
          (B = s.from(A.maskPattern)),
          A.toSJISFunc && r.setToSJISFunction(A.toSJISFunc)),
        N(p, y, l, B)
      );
    }),
    nt
          ...
          (r.qrToImageData = function (n, t, e) {
            ...
            for (let E = 0; E < c; E++)
              for (let g = 0; g < c; g++) {
                let P = (E * c + g) * 4,
                  f = e.color.light;
                if (E >= m && g >= m && E < c - m && g < c - m) {
                  const M = Math.floor((E - m) / u),
                    h = Math.floor((g - m) / u);
                  f = S[a[M * s + h] ? 1 : 0];
                }
        ...
        (r.render = function (e, s, a) {
          ...
          (r.renderToDataURL = function (e, s, a) {
            ...
            const c = r.render(e, s, u),
              m = u.type || "image/png",
              S = u.rendererOpts || {};
            return c.toDataURL(m, S.quality);
          });
      })(pt)),
    pt
  );
  ...
  function t(e, s, a, u, c) {
    ...
    if (E) {
      ...
      S === 2
        ? ((c = a), (a = s), (s = u = void 0))
        : S === 3 &&
          (s.getContext && typeof c == "undefined"
            ? ((c = u), (u = void 0))
...
const Ge = me(Qe),
  ...
  $e = we({
    ...
    setup(r, { emit: o }) {
      const { toCanvas: i, toDataURL: n } = Ge,
        t = Nt(!0),
        e = Nt(null),
        s = vt(() => String(r.text)),
        a = vt(() => ({ width: r.width + "px", height: r.width + "px" })),
        u = () =>
          tt(null, null, function* () {
            yield Re();
            const f = Ae(r.options || {});
            if (r.tag === "canvas") {
              f.errorCorrectionLevel = f.errorCorrectionLevel || S(V(s));
              const M = yield m(V(s), f);
              f.scale = r.width === 0 ? void 0 : (r.width / M) * 4;
              const h = yield i(V(e), V(s), f);
              if (r.logo) {
                const b = yield c(h);
                o("done", b), (t.value = !1);
              ...
              const M = yield n(
                s.value,
                Mt({ errorCorrectionLevel: "H", width: r.width }, f)
              );
              (V(e).src = M), o("done", M), (t.value = !1);
            }
          });
      Ce(
        () => s.value,
      ...
      const c = f => {
          const M = f.width,
            ...
            d = f.getContext("2d");
          if (!d) return;
          E(d)(T, T, B, B, w), (d.fillStyle = R), d.fill();
          const I = new Image();
          (N || p) && I.setAttribute("crossOrigin", N), (I.src = A);
          const v = F => {
              d.drawImage(F, y, y, l, l);
            },
            k = F => {
              const z = document.createElement("canvas");
              (z.width = y + l), (z.height = y + l);
              const Q = z.getContext("2d");
              if (
                !Q ||
                !d ||
                (Q.drawImage(F, y, y, l, l), E(d)(y, y, l, l, p), !d)
              )
                return;
        ...
        m = (f, M) =>
          tt(null, null, function* () {
            ...
            return yield i(h, f, M), h.width;
          }),
        S = f => (f.length > 36 ? "M" : f.length > 16 ? "Q" : "H"),
        E = f => (M, h, b, R, C) => {
          const N = Math.min(b, R);
          return (
            C > N / 2 && (C = N / 2),
            f.beginPath(),
            f.moveTo(M + C, h),
            f.arcTo(M + b, h, M + b, h + R, C),
            f.arcTo(M + b, h + R, M, h + R, C),
            f.arcTo(M, h + R, M, h, C),
            f.arcTo(M, h, M + b, h, C),
            f.closePath(),
            f
